 ...
 */
void setup_font(){
    set_bkg_data(FONT_MEMORY_START, FONT_SIZE, font_tiles);
}
...
void clear_text(UINT8 x, UINT8 y, UINT8 w, UBYTE surface){
    if(surface){
        set_bkg_tiles(x,y,w,1,&font_tiles[FONT_BLANK]); // clear entire bkg row
    }else{
        set_win_tiles(x,y,w,1,&font_tiles[FONT_BLANK]); // clear that section of window
    }
}